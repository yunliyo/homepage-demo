...
/*fetch('https://ipapi.co/json/')
    ...
    .then(data => {
        ...
        // 使用用户的地理位置信息获取天气
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${userCity},${userRegion},${userCountry}&units=metric&appid=88a2faa5bacb1fb75c24b84d7fb5d932`)
            .then(response => {
                if (!response.ok) {
                    throw new Error('Network response was not ok');
                }
            ...
            .then(data => {
                $('#wea_text').text(data.weather[0].description);
                $('#tem1').text(data.main.temp_min);
                $('#tem2').text(data.main.temp_max);
            })
            .catch(error => {
                console.error('There has been a problem with your fetch operation:', error);
                // 在这里可以添加一些错误处理逻辑，例如显示一个默认的天气信息或者提示用户检查网络连接
                $('#wea_text').text('无法获取天气信息');
                $('#tem1').text('');
                $('#tem2').text('');
            });
    })
...
`
var title1 = 'Yunli Search'
var title2 = `Yunli Search Pro`
var content = `
版 本 号：1.1
更新日期：2022-07-12
`
